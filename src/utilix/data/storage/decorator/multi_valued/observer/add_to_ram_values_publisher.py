from typing import runtime_checkable, Protocol, Any
from utilix.data.storage.decorator.multi_valued.observer.add_to_ram_values_subscriber import AddToRamValuesSubscriber

@runtime_checkable
class AddToRamValuesPublisher(Protocol):
    def attach_add_to_ram_values_subscriber(self, add_observer: AddToRamValuesSubscriber) -> None: ...
    def dettach_add_to_ram_values_subscriber(self, add_observer: AddToRamValuesSubscriber) -> None: ...
    def notify_add_to_ram_values_subscribers(self, value:Any) -> None: ...
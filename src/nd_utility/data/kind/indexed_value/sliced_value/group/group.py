...
class Group:
    ...
    def get_the_broadest_one(self) -> SlicedValues:
        """
        Broadest range ignoring overlap_size compatibility.
        Step is set to 1.
        """
        ...
        for vs in self._sliced_values_list:
            ...
            if stop is None:
                # Infer stop from values length and overlap_size
    ...
    def get_one_steped_combined(self) -> SlicedValues:
        """
        Combine all slices into a single SlicedValues with overlap_size=1.
    ...
    def get_combined(self) -> SlicedValues:
        """
        Try to return a combined SlicedValues that preserves a common overlap_size
        only if all steps are equal and aligned. Otherwise fallback to overlap_size=1.
        """
        ...
        # Safe to preserve common overlap_size
        index_to_value: Dict[int, Any] = {}
        for vs in self._sliced_values_list:
            for idx, val in vs.iter_index_value():

...
from nd_utility.data.kind.dic.decorator.decorator import Decorator as DicDecorator
...
class UniquenessChecked(DicDecorator):
    ...
    def validate_unique_items_in_lists(
        ...
        PublisherExample:
            >>> group = {"dims": {"time": {"units": ["second", "minute", "minute"]}}}
            >>> ok, problems = validate_unique_items_in_lists(group)
            >>> ok
            False
            >>> problems[0]["str_path"]
            'dims!time!units'

...
from nd_utility.data.kind.dic.interface import Interface as DicInterface
import io
...
class Dic(DicInterface):
    ...
    def get_child_by_name(self, name: str) -> "Dic":
        """
        Depth-first search (pre-order) from the root.

        - Finds the first key whose name equals `name`.
        - Returns a NEW Dic that contains the subtree starting from THAT NODE'S CHILDREN down to leaves.
          (So the returned Dic corresponds to the matched key's value.)
        - If the matched key is a leaf (non-dict), returns an empty Dic.
        """
        ...
        root = unwrap(self._raw_dict)
        if not isinstance(root, dict):
            raise TypeError(f"Root must be dict-like, got {type(root).__name__}")

        # Stack holds (current_node_dict, iterator over its items) for explicit DFS.
        stack: List[Tuple[dict, Any]] = [(root, iter(root.items()))]

        while len(stack) > 0:
            current_dict, current_iter = stack[-1]
            ...
            if key == name:
                value_unwrapped = unwrap(value)
                if isinstance(value_unwrapped, dict):
                    return Dic(value_unwrapped)
                return Dic({})
